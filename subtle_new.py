...
class Find(QMainWindow):
    ...
    def initFindUI(self):
        try:
            ...
            #setting window icon
            self.setWindowIcon(QIcon('snake.jpeg'))
...
# mayuresh window for uploading file
...
class UploadWindow(QMainWindow):
    ...
    def showdialog(self,text):
        ...
        msg.setWindowIcon(QIcon('snake.jpeg'))
        msg.exec()
    ...
    def OpenFileToUpload(self):
        ...
        currentfile = os.path.basename(filename[0])
        filepath = filename[0]
        print(filepath)
        print(currentfile)
        currentdir = os.getcwd().replace("\\","/")
        print(currentdir)
        print(currentdir+"/"+currentfile)
        if currentdir+"/"+currentfile == filepath:
           self.lineEdit_4.setText(currentfile)
        elif currentdir+"/"+currentfile != filepath: 
    ...
    def UploadToGithub(self):
        try:
            ...
            #self.showdialog('Incorrect UserName/Password/RepositoryName')
            print('Exception in upload to git hub in upload class:',str(sys.exc_info()))
...
#to change the color of main window title and borders
    ...
    def initUI(self):
        ...
        global t
        t = threading.Timer(3.0,setTime,[self])
        t.start()
...
#so our application is shown
            #self.centralWidget.destroyed.connect(self.handleExit)
            self.show()
        except Exception as e:
...
#these functions are defined so that save all file function can call them and
#save the file according to the index respectively

    def saveAllSave(self,index):
        try:
            print('in save all save')
            name = self.tabs.tabText(index)
            if name == 'UnTitled*':
                self.saveAllSaveAs(index)
            else:
                name = name.replace('*','')
                path = myadd.get(name)
                widget = QWidget()
            ...
            print('Exception in save all save:',str(sys.exc_info()))


    def saveAllSaveAs(self,index):
        try:
            widget = self.tabs.widget(index)
            grid = widget.layout()
            lay = grid.itemAtPosition(0,0)
...
#wants to save it or not
                ...
                if tabName.endswith('*'):
                    print('ends with *')
                    reply = QMessageBox.question(self,'Continue?','Do you want to save the file or not?', QMessageBox.Yes, QMessageBox.No)
...
#where we will check if the file in the tab is saved or not
        try:
            ...
            print('Exception n close all tabs : ',str(sys.exc_info()))
...
#for closing the whole application
    ...
    def closeEvent(self,event):
        try:
            ...
            print(event)
            print(self)
            reply = QMessageBox.question(self,'Continue?','Do you want to Quit?', QMessageBox.Yes, QMessageBox.No)
            if reply == QMessageBox.Yes:
                print('yes')
                self.exit()
            ...
            print("Closing....",str(sys.info_exc()))
...
#mayuresh function for uploading
    def uploadFile(self):
...
#aditya method for voice
    def voicecode(self):
         ...
         while bool == True:
            ...
            if 'exit from voice mode' == text:
                bool = False
                return
            elif 'voice mode exit' == text:
                  bool = False
                  return
            else :
                  self.information()
                  ...
                  #self.file.close()
                  #self.WriteIntoEditor()

    def WriteIntoEditor(self):
        file1 = open("temp.txt","r")
        data=file1.read()
        self.information()
        self.globaltext.append(data)
        #self.update()
    ...
    def voiceoperation(self):
         ...
         if string == 'exit from voice mode':
         	return
         elif string is None:
          	return
         else :
...
# mayuresh suggestions for spotllight search
    def suggest(self):
        try:
            print('suggestion?')
...
#called every 30 seconds so the user does not have to save the named files all the time
    def autoSave(self):
        try:
            #print('auto')
            cnt = self.tabs.count()
            for i in range(cnt):
                ...
                #print(name)
                if name != 'UnTitled*' and name != 'UnTitled'and name.find('*'):
                    #print('to save')
                    name = self.tabs.tabText(i)
                    name = name.replace('*','')
                    #print('new name ',name)
            ...
            print('Exception in auto save:',str(sys.exc_info()))
...
def setTime(self):
    ...
    global t
    self.autoSave()
    t = threading.Timer(30.0,setTime,[self])
    t.start()
...
class StringFromRecognizer:
      ...
      def ToHTMLcode(self,GotSentence):
           ...
           yes_or_no = 'yes'
           #i=0
           #array for storing the attributes and the value
           attarr = []
           attvalue = []
                         ...
                         if said == 'yes':
                            ...
                            for attrkey in attribute_keys:
                                ...
                                if attrkey == attsaid:
                                   ...
                                   break
                          #if said no then dont add any attribute and wont ask for attribute and will form the string ie html code
                         elif said == 'no':
                              formedString2=""
                              chevrons="<>"
...
# from here control goes to StringFromRecognize to do actual forming the code from text we got from voice
class VoiceControl :
      def __init__(self):
         print("Does nothing")
      ...
      def voicecodeMain(self):
             ...
             if variable is None:
                ...
                #self.Speak('Say option again')
                   ...
                   bool = False
                   print("You have exited from voice mode.")
                   return 'exit from voice mode'
                   ...
                   return code
             elif 'tag open' in variable:
                   print("inside 4 if")
                   htmlcode = object.ToHTMLcode(variable)
                   return htmlcode
             elif 'tag close' in variable:
                   print("inside 4 if")
                   htmlcode = object.ToHTMLcode(variable)
                   return htmlcode
             else:
                   ...
                   return justwrite
#this function is to pass control above class to do actual operation
...
if __name__=='__main__'    :
	#wait for the starting window for 3 seconds until that window closes,creates a subprocess to run that starter window for 3 seconds
